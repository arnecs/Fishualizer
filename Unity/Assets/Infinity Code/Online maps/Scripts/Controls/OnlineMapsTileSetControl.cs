...
public class OnlineMapsTileSetControl : OnlineMapsControlBase3D
{
    ...
    private void UpdateMapMesh()
    {
        ...
        for (int y = 0; y < h; y++)
        {
            for (int x = 0; x < w; x++)
            {
                UpdateMapSubMesh(x, y, w, h, subMeshSizeX, subMeshSizeY, subMeshVX, subMeshVZ, startPosX, startPosZ, yScale, tlx, tly, brx, bry);
            }
        }
    ...
    private void UpdateMapSubMesh(int x, int y, int w, int h, double subMeshSizeX, double subMeshSizeY, int subMeshVX, int subMeshVZ, double startPosX, double startPosZ, float yScale, double tlx, double tly, double brx, double bry)
    {
        int mi = x + y * w;
        ...
        Material material = rendererInstance.materials[mi];
